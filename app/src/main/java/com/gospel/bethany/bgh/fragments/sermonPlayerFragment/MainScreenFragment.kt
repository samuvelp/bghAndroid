package com.gospel.bethany.bgh.fragments.sermonPlayerFragment
...
//
//        bottomBarSetup()
        Helper.getSermons().onSuccess { task ->
            getSongsList = task.result
            val prefs = activity!!.getSharedPreferences("action_sort", Context.MODE_PRIVATE)
            val action_sort_ascending = prefs.getString("action_sort_ascending", "true")
            val action_sort_recent = prefs.getString("action_sort_recent", "false")
...
//    }
    ...
    fun getSongsFromPhone(): ArrayList<Songs> {
        ...
        if (songCursor != null && songCursor.moveToFirst()) {
            ...
            while (songCursor.moveToNext()) {
                ...
                if (currentTitle != null && currentArtist != null && currentData != null)
    ...
    fun getSongs(): ArrayList<Songs> {
        var arrayList = ArrayList<Songs>()
        var currentId = 12345000000
        var currentTitle = "Auto rickshaw diss track"
        var currentArtist = "Sam"
        var currentData = "http://file-examples.com/wp-content/uploads/2017/11/file_example_MP3_700KB.mp3"
        var currentDate = 1543139905000
        arrayList.add(Songs(currentId, currentTitle, currentArtist, currentData, currentDate))
        return arrayList
    }

    fun getSermonFromFirebase(): ArrayList<Songs> {
        var arrayList = ArrayList<Songs>()
        Helper.getSermon().continueWith { task ->
            if (task.isCompleted) {
